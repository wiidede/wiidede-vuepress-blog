...
const collectFontSizePath = (option) => {
  ...
  const basePathList = pathList.filter((val) => val.endsWith('.name') || val.endsWith('id'))
  const fontSizePathList = pathList.filter((val) => val.endsWith('.fontSize'))
