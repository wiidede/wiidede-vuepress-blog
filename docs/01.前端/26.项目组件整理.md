...
permalink: /project-components-collection/
...
export default {
	...
	methods: {
		getIconCls() {
			// let cls = `iconfont kc-icon-${this.type} icon-${this.type}`;
  ...
  <div>
    <div class="collapse-title" :class="titleClass">
      <div class="title" @click="changeCollapse">
        <span>{{ title }}</span>
        <i class="kc-icon-arrow-down" :class="{'is-unfold' : !isCollapse}"></i>
      </div>
  ...
  <div class="common-h1" :class="{'outer-slot': outerSlot}">
    <div>
      ...
      <div class="common-h1-subtitle">
        {{ subtitle }}
        <div v-if="hasSlot" class="tool-slot kiaf3-tool-bar">
          <slot></slot>
        </div>
      </div>
    </div>
